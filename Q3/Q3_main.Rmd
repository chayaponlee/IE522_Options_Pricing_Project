...
\begin{center}\textit{Table 3.1: Computational Speed Comparison (in units of prices/second) for n=256000 and k=3}\end{center}
```{r, echo=FALSE}
...
\begin{center}\textit{Table 3.2: Runtime Comparison for n=256000 and k=3}\end{center}
...
```
\begin{center}\textit{Figure 3.1: Computational Speed Improvements}\end{center}
```{r, echo=FALSE}
logy1 = log10(y1)
logy2 = log10(y2)
...
```
\begin{center}\textit{Table 3.3: Put Option Value, Standard Error, and Run time}\end{center}
```{r,echo=FALSE}
...
```
\begin{center}\textit{Table 3.4: Early Exercise Value, Percentage of Early Exercise}\end{center}
```{r,echo=FALSE}
kable(put_LSM_results2, "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("scale_down","striped", "hold_position"))
```

\begin{center}\textit{Figure 3.2: Options Convergence path when k=3}\end{center}
```{r, echo=FALSE}
...
\begin{center}\textit{Table 3.5: LSM price accuracy with BSSR price as benchmark}\end{center}
```{r, echo=FALSE}
...
\begin{center}\textit{Table 3.6: Accuracy Comparison with True Value as benchmark}\end{center}
```{r, echo=FALSE}
V_binom = 2.225874
put_LSM_results_fast = read.csv("put_LSM_cpf_res_20241127.csv", header=TRUE, row.names=1)
m_steps = c(10,20,40,80)
...
\begin{center}\textit{Table 3.7: Run Times (seconds/price) and Speed Comparison (prices/sec)}\end{center}
```{r,echo=FALSE}
m_steps = c(10,20,40,80)
...
\begin{center}\textit{Figure 3.4: Speed vs Abs. Error Plot Comparison}\end{center}
```{r,echo=FALSE}
...
```
\newpage
\textbf{3.3: Code Snippet of Binomial Model with n=15000 time steps}
```{r, eval=FALSE}
