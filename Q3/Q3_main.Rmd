...
\section*{3. Least-Squares Monte Carlo Algorithm (LSM) on Put Option}

\textbf{3.1 Objective and Motivation}

In this section, we researched and implemented the LSM algorithm based on the concepts and method depicted in the paper “Valuing American Options by Simulation: A Simple Least-Squares Approach” by Longstaff and Schwartz. The Least Squares Monte Carlo (LSM) approach is powerful in pricing American options as it estimates the conditional expectation of continuation payoffs, which is central to determining the optimal exercise strategy. This aligns with the fundamental asset pricing theorem, where the conditional expectation under the risk-neutral measure ensures that the stock price paths  \(S_t^* = S_t e^{-rt}\)  are modeled as martingales, reflecting no-arbitrage conditions. By leveraging this martingale property and cross-sectional regression on simulated paths, LSM accurately values American options by capturing the fair price of the option’s continuation payoff.

We emphasize that the purpose of this report is not to summarize and explain in detail what Longstaff and Schwartz has implemented nor restate the concepts behind LSM. Our primary objective is to iteratively run LSM to calculate the expected put option value on different settings, by using different combination of cartesian products between the number of stock paths (n), number of regressors (k), and number of time steps (m).  We will observe and present our findings and examine the effects of varying these parameters as we run our simulations. Through out the report, we will introduce the usage of variance reduction techniques and (our own) choices of data processing where appropriate. One potential downside to using the LSM algorithm is if naively implemented, it will be computationally expensive as the number of paths and number of time steps increase in tandem due to the need for performing least-squares in every recursive step. We will present our efforts in reducing the computational run time of the algorithm and show that if carefully programmed, the LSM algorithm is certainly admissible in practice.

\textbf{3.2 Generating Stock Path}
...
\begin{center}\textit{Snippet 3.1: Generating stock path matrix}\end{center}
```{r}
...
\textbf{3.3 Choice of Basis Function and Scaling Stock Prices}

In our experiment, we ran our least squares method by representing our regressors as basis functions calculated from our underlying stock prices \(S_t = {S_t^1, S_t^2,....., S_t^n}\) for each time step $t$. Although our primary analysis will be based on using Chebshev Polynomials as our basis function, we did run simulations on Laguerre Polynomials (as suggested in the paper) as well. We concluded that the converged put option value are very similar to each other and the discrepancy between the two choices of basis functions are very small. Both polynomials are orthogonal which helps to reduce collinearity between the regressors and we omit the use of regular polynomials since they are prone to collinearity. Using Chebyshev and Laguerre polynomials has proven to offer numerical stability in performing lease squares regression. For interested readers, we include our LSM results in Appendix 3.2.

Before applying Chebyshev polynomial conversions on the stock prices, we apply min-max scaling with boundary $[-1,1]$ since Chebyshev polynomials are orthogonal between $[-1,1]$ and minimizes the approximation error within these boundaries. For Laguerre polynomials, we apply min-max scaling with boundary $[0,1]$, since they are designed to take positive values $[0,\inf]$, but because they consist of exponential terms, we cap our normalized value within 1, ensuring numerical stability. 

We demonstrate our implementations of our basis functions and scaling functions in Snippet 2. 

\begin{center}\textit{Snippet 3.2: Basis Functions and min-max scaler}\end{center}
```{r}
...
\textbf{3.4 Least Squares Monte Carlo Algorithm}
...
\newpage
\begin{center}\textit{Snippet 3.3: LSM Algorithm Implementation}\end{center}
```{r}
...
\textbf{3.5 Computational Efficiency Improvements}

In this section, we explain our implementations in increasing the computational efficiency and speed of our LSM algorithm. Our original implementation of the algorithm consists primarily of manipulating dataframe objects and passing our regressorss dataframe into R's "lm" least squares package. This method have proven to be underwhelming in terms of computational speed and allocates unnecessary memory overhead in R (in fact, in any scientific programming language). For interested readers, the original implementation of the algorithm, including basis function implementations can be found in Appendix 3.1. But we argue that it is preferred to start prototyping our algorithm with dataframes initially because it is easier to debug and more intuitive for the developer. For example, we can check our least squares result to examine whether or not numerical instability persists and coefficients produced are stable or not. 

After ensuring that our algorithm computations are stable and the results are sound, we shifted our approach from working with dataframes to matrices, as seen in Snippet 3.2 and 3.3 where the entirety of our code involves only matrix computations. In particular, we discarded the "lm" package and implemented our own least squares function where we solved for the coefficients using the R function "solve" and computing the dot product between the Chebyshev polynomials and coefficients. This proved to be a huge improvement in computational efficiency. 

We compare the computational speed between the "dataframe" and "matrix" implementations with three regressors in Table 1 and Figure 1 . Table 1 shows the speed in terms of option prices (option paths) evaluated per second as the number of $m$ time steps increase. Figure 1 illustrates this in log10 scaling. As an example, for $m=80$, the speed increased from 1367.92 to 80060.05 options prices per second. This means that for $n=256000$, the corresponding runtime will have decreased from around 187.1 to 3.2 seconds. As we will see later, for $n=256000$, the standard error of the options price is very small and hence at 3.2 seconds runtime, we think that this is potentially acceptable in practice. We compare the runtimes for $n=256000$ and three regressors in Table 2. 
...
\begin{center}\textit{Table 1: Computational Speed Comparison (in units of options/second)}\end{center}
```{r, echo=FALSE}
...
\begin{center}\textit{Table 2: Runtime Comparison for n=256000 and k=3}\end{center}
...
```
\begin{center}\textit{Figure 1: Computational Speed}\end{center}
```{r, echo=FALSE}
logy1 = log10(y1)
logy2 = log10(y2)
...
plot(m_steps,logy1, type = "b", col = "blue", lwd = 2, ylim = range(c(logy1, logy2)),
     ...
     main = "LSM Computational Speed")

lines(m_steps, logy2, type="b", col = "red", lwd = 2)
```

\textbf{3.6 Experiment Implementation}
...
\begin{center}\textit{Snippet 3.4: BSM Closed Form Function}\end{center}
```{r}
...
```
\begin{center}\textit{Snippet 3.5: Experiment Implementation}\end{center}
```{r, eval=FALSE}
...
\textbf{3.7 Result Analysis}

Table 3 shows the LSM simulations results for every combination of $n$'s, $m$'s, and $k$'s stated in 3.6 and includes put option values (value), standard errors (se), and run times (rt) and Table 4 shows the results for early exercise values (EE_value) and percentage of options that were early exercised (Pct_EE). We observed that $\forall k$, the standard error of the put value decreases as we increase $n$. Since we choose $n$ such that for every iteration $n$ increases by 4 times, the standard error of decreases by 2 times, as standard error converges at a rate of order $\frac{1}{\sqrt{n}}$. Although not so obvious, as $m$ increases, we observe that the standard error also decreases but by a very marginal amount. Run time for the simulations also increase as $m$, $n$, $k$ increases. In particular, $\forall n \in [4000,16000]$, we were able to compute put values that have standard errors approximately between 4 to 2 cents, which is certainly admissible for American Options. Figure 2 shows the convergence path of the put values as $n$ increases for difference $m$ parameters fixing $k=3$. We can see that for $n<64000$ the convergence path seems to be oscillating quite significantly. For $n > 64000$, we get a very smooth converging path. For $m = 40,80$, the both path converges to a very similar value, which is in line with the fact that as $m$ increases, we get a more accurate result. We will discuss about accuracy of our put value in 3A where we also compute the put option value via Binomial Black-Schoels with Richarson Exprapolation.

Moving on to table 4, we see the effects of including higher order polynomials (increasing $k$). All else equal, both the early exercise value and percentage of early exercise tends to increase. Increasing the order of polynomial basis in LSM improves the accuracy of continuation value estimation, allowing for better identification of early exercise opportunities and capturing complex, nonlinear relationships between stock price and payoffs. This leads to higher early exercise value and percentage of early exercised options. This is also inline with the fact that the early exercise value should positive for dividend paying stocks. Increasing $m$ also increases the percentage of early exercise which comes from the increasing granularity of capturing accurate continuation payoffs. Fixing $m$ and $k$, we see that increasing $n$ reduces the variability in early exercise value and percentage of early exercise. The values stabilize and tend toward convergence as $n$ becomes large.
...
```
\begin{center}\textit{Table 3: Put Option Value, Standard Error, and Run time}\end{center}
```{r,echo=FALSE}
...
```
\begin{center}\textit{Table 4: Early Exercise Value, Percentage of Early Exercise}\end{center}
```{r,echo=FALSE}
kable(put_LSM_results2, "latex", booktabs = TRUE) %>%
  kable_styling(latex_options = c("scale_down","striped", "hold_position"))
```

\begin{center}\textit{Figure 2: Options Convergence path when k=3}\end{center}
```{r, echo=FALSE}
...
\begin{center}\textit{Snippet 3.6: Experiment Implementation}\end{center}
```{r}
...
\textbf{3A.3 Discussion on Accuracy, Path Convergence, and Computational Efficiency}

\textbf{Accuracy}

In this section we compare accuracy, convergence behavior and computational speed of LSM($k=3,n=64000$), LSM($k=3,n=256000$), and BBSR methods with varying $m$ steps where \(m:\{10,20,40,80\}\). We purposely chose LSM($k=3,n=256000$) to reflect the most precise and stable method among all LSM runs and LSM($k=3,n=64000$) to sacrifice some precision for speed. This will provide us with a more meaningful comparison once we include the BBSR method into the picture. We use the absolute error as our accuracy measure and we present this along with the corresponding option values in Table 5. We also include the true value in the table as well. We can see that the BBSR method is very accurate and superior to the LSM method at every $m$. At $m=80$, the error of the BSSR value is small and negligible in practice. This is not to say that the LSM algorithm did a poor job, because we can see that the absolute error is of order $10^{-3}$, which is certainly less than a cent. 

\begin{center}\textit{Table 5: Accuracy Comparison with True Value as benchmark}\end{center}
```{r, echo=FALSE}
V_binom = 2.225874
put_LSM_results_fast = read.csv("put_LSM_cpf_res_20241127.csv", header=TRUE, row.names=1)
m_steps = c(10,20,40,80)
...
\begin{center}\textit{Figure 4: Speed vs Abs. Error Plot Comparison}\end{center}
```{r,echo=FALSE}
...
plot(e1, s1, type = "n", col = "blue", lwd = 2, ylim=range(c(s1,s2,s3)),
     ...
     main = "Comparison of Computational Speec (Option prices/sec) vs Abs. Error")
...
\textbf{3.1: Non-vectorized version of LSM Algorithm using DataFrames}
```{r}
...
\textbf{3.2: LSM Results using Laguerre Basis Functions}
...
\textbf{3.3: LSM Results using Laguerre Basis Functions}
```{r, eval=FALSE}
